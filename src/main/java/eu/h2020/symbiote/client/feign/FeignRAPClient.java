...
public class FeignRAPClient extends AbstractSemanticRAPClient implements RAPClient {
    ...
    public FeignRAPClient(ISecurityHandler securityHandler, CRAMClient cramClient, SearchClient searchClient) {
        ...
        super.setRAPClient(this);
        this.securityHandler = securityHandler;
        logger.debug("##securityHandler = "+securityHandler);
        logger.debug("##cramClient = "+cramClient);
        logger.debug("##searchClient = "+searchClient);
    }
    ...
    @Override
    public List<Observation> getObservationsByQuery(String resourceUrl, int top, String from_date, String to_date, boolean serverValidation, Set<String> homePlatformIds) {
        try {
            logger.debug("##resourceUrl="+resourceUrl);
            RAPI rapClient = getClient(resourceUrl);
            return rapClient.getTopObservationsByQuery(top, from_date, to_date, serverValidation, homePlatformIds);
        } catch (SecurityHandlerException e) {
            logger.error("Could not get Observations", e);
            return null;
        }
    ...
    interface RAPI {
        ...
        @RequestLine("GET /Observations?$top={top}&from_date={from_date}&to_date={to_date}")
        @Headers({"Accept: application/json", "Content-Type: application/json",
                SERVER_VALIDATION_HEADER + ": {serverValidation}", HOME_PLATFORM_IDS_HEADER + ": {homePlatformIds}"})
        List<Observation> getTopObservationsByQuery(
                @Param("top") int top,

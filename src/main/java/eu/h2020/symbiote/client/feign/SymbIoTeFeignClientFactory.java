...
public class SymbIoTeFeignClientFactory extends AbstractSymbIoTeClientFactory {
    ...
    public RAPClient getRapClient() {
        ISecurityHandler securityHandlerLib = securityHandler;
        logger.debug("##securityHandlerLib = "+securityHandlerLib);
        CRAMClient crmClient = getCramClient();
        logger.debug("##crmClient = "+crmClient);
        SearchClient searchClient = getSearchClient();
        logger.debug("##searchClient = "+searchClient);
        return new FeignRAPClient(securityHandlerLib, crmClient, searchClient);
    }
